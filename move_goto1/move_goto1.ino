...
void setup() {
  ...
  Serial.println("System Ready. Enter a floor (0 for 1st floor, 1 to 4).");
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    else if (targetFloor == 0) { // 0 입력 시 1층 이동
      Serial.println("Moving down to 1st floor...");
      moveToFirstFloor(); // 1층으로 이동
    } 
    else {
      Serial.println("Invalid floor. Enter a number between 0 and 4.");
    }
  }
}
...
void moveToFirstFloor() {
  digitalWrite(DIR, LOW); // 아래로 이동 설정
  isMoving = true;
  ...
  while (isMoving) {
    ...
    // 센서 4번 (1층) 감지 시 중지
    if (digitalRead(sensorPins[0]) == LOW) { // sensorPins[0] = 4번 핀
      Serial.println("Arrived at 1st floor.");
      stopMotor(); // 모터 정지
      isMoving = false; // 이동 중단
    }
  }
